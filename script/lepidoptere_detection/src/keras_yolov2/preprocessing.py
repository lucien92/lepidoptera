...
class BatchGenerator(Sequence):
    ...
    def __len__(self):
        # print("nombre step", int(np.ceil(float(len(self._images)) / self._config['IMG_PER_BATCH'])))
        # print("nombre images du train", len(self._images))
        #on veut afficher que la valeur des clés filename du dictionnaire
        # for i in range(len(self._images)):
        #     print("les images", self._images[i]['filename'])
    ...
    def on_epoch_end(self):
        ...
        # Shuffle raw images
        if self._shuffle:
            #print("nombre d'images avant shuffle", len(self._raw_images))
            np.random.shuffle(self._raw_images)
        
        ## Use raw images as image set
        if not self._sampling:
            self._images = self._raw_images
            # print("nombre d'images distinctes", len(self._raw_images))
            # print("nombre d'images sans sampling", len(set(self._images))) #on remarque que les 396 images sont bien toutes distinctes. Pk écart entre 433 et 396?
            return
